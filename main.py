...
async def CalendarSendProgress(files,jdb,bot,update,message):
    inf = jdb.get_user(update.message.sender.username)
    user = inf['moodle_user']
    passw = inf['moodle_password']
    host = inf['moodle_host']
    proxy = inf['proxy']
    urls = []
    for fi in files:
        urls.append(fi['directurl']+"\n")
    cd = Draft2Calendar()
    try:
        sending = await cd.send_calendar(host,user,passw,urls,proxy)
        print("ok")
        mcl = moodlews.get_webservice_token(host=host,username=user,password=passw,proxy=proxy)
        login_token = mcl 
        urlcalendar = []
        print("ok1")
        for url in sending:
            singed = str(sign_url(login_token,URL(url)))
            urlcalendar.append(singed+"\n")
        print("ok2")
        return urlcalendar    
...
def processFile(update,bot,message,file,thread=None,jdb=None):
    ...
    if data:
        ...
        send_calendar = asyncio.run(CalendarSendProgress(files,jdb,bot,update,message))
        if send_calendar:
          print(send_calendar)  
          try:
            finishInfo = infos.createFinishUploading(file,file_size)
            filesInfo = infos.createFileMsg(file,send_calendar)    
            bot.sendMessage(message.chat.id,finishInfo+'\n'+filesInfo,parse_mode='html')
            if len(send_calendar)>0:
                txtname = str(file).split('/')[-1].split('.')[0] + '.txt'
                sendTxt(txtname,send_calendar,update,bot)      
          except Exception as ex:
            print(ex) 
        else:
         finishInfo = infos.createFinishUploading(file,file_size)
...
def main():
    ...
    print('init bot.')
    #set in debug
    bot_token = '5770670380:AAGKwNho5iML_EXSTQkaVQYhsoLoIh_OAxs'
